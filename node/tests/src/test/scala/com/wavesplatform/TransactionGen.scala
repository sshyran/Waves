...
trait TransactionGenBase extends ScriptGen with TypedScriptGen with NTPTime { _: Suite =>
  ...
  def addressGen(chainId: Byte = AddressScheme.current.chainId): Gen[Address] =
    byteArrayGen(Address.HashLength).map { hashBytes =>
      val withoutChecksum = Bytes.concat(Array(Address.AddressVersion, chainId), hashBytes)
      Address.createUnsafe(Bytes.concat(withoutChecksum, crypto.secureHash(withoutChecksum).take(Address.ChecksumLength)))
    }
  ...
  def constOrGen[A](value: Option[A], gen: => Gen[A]): Gen[A] = value.fold(gen)(Gen.const)

  def transferGen(
      timestamp: Long,
      version: Option[TxVersion] = None,
      sender: Option[PublicKey] = None,
      recipient: Option[AddressOrAlias] = None,
    ...
    for {
      version    <- constOrGen(version, Gen.oneOf(TransferTransaction.supportedVersions.toSeq))
      sender     <- constOrGen(sender, byteArrayGen(crypto.KeyLength).map(bs => PublicKey(bs)))
      recipient  <- constOrGen(recipient, accountOrAliasGen)
      asset      <- constOrGen(asset, assetIdGen.map(Asset.fromCompatId))
  ...
  def tgp(
      ...
      maxAmount: Long = 100000000L * 100000000L / 100,
      asset: Asset = Waves,
      feeAsset: Asset = Waves
  ): Gen[TransferTransaction] =
    for {
      amount     <- Gen.choose(1, maxAmount)
      tx <- transferGen(timestamp, Some(version), Some(sender.publicKey), Some(recipient), Some(asset), Some(amount), Some(feeAsset))
    } yield TransferTransaction.sign(tx, sender.privateKey)
  ...
  def transferGeneratorP(sender: KeyPair, recipient: AddressOrAlias, assetId: Asset, feeAssetId: Asset): Gen[TransferTransaction] =
    ...
    } yield SignedTx.transfer(1.toByte, sender, recipient, assetId, amount, feeAssetId, feeAmount, attachment, timestamp)
  ...
  def transferGeneratorP(timestamp: Long, sender: KeyPair, recipient: AddressOrAlias, assetId: Asset, feeAssetId: Asset): Gen[TransferTransaction] =
    tgp(1.toByte, timestamp, sender, recipient, asset = assetId, feeAsset = feeAssetId)
  ...
  def createWavesTransfer(
    ...
    SignedTx.transfer(1.toByte, sender, recipient, Waves, amount, Waves, fee, None, timestamp)
  ...
  } yield SignedTx.transfer(1.toByte, sender, recipient, assetId, amount, Waves, feeAmount, attachment, timestamp)
  ...
  } yield SignedTx.transfer(1.toByte, sender, sender.toAddress, assetId, amount, Waves, feeAmount, attachment, timestamp)
  ...
  } yield SignedTx.transfer(1.toByte, sender, sender.toAddress, assetId, amount, feeAssetId, feeAmount, attachment, timestamp)
