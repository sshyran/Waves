...
import com.wavesplatform.transaction.{Asset, GenesisTransaction, SignedTx, TxValidationError}
...
class TransferTransactionDiffTest extends PropSpec with PropertyChecks with WithState with TransactionGen with NoShrink {
  ...
  property("handle transactions with amount + fee > Long.MaxValue") {
    val precs = for {
      ...
      transfer = SignedTx.transfer(1.toByte, master, recepient.toAddress, asset, Long.MaxValue, Waves, 100000, None, ts)
