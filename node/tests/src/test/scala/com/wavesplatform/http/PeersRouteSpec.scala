...
class PeersRouteSpec extends RouteSpec("/peers") with RestAPISettingsHelper with PropertyChecks with MockFactory {
  ...
  routePath("/connect") in {
    ...
    val result = Post(connectUri, ConnectReq(address.getHostString, address.getPort)) ~> ApiKeyHeader ~> route ~> runRoute
    check {
      responseAs[ConnectResp].hostname shouldEqual address.toString
    }(result)
  }
