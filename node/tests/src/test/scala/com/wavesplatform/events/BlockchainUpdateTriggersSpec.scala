...
import com.wavesplatform.transaction.{BlockchainUpdater, DataTransaction, GenesisTransaction, SignedTx, Transaction}
  ...
  "appends correctly" - {
    ...
    "block/microblock with balance updates from transfer txs" in forAll {
      for {
        ...
        sender2recipient = SignedTx.transfer(2.toByte, sender, recipient.toAddress, Waves, master2sender.amount - fee, Waves, fee, None, 2)
