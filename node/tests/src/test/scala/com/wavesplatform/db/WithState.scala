...
import com.wavesplatform.database.{Storage, TestDB, TestStorageFactory}
...
trait WithState extends DBCacheSettings with Matchers with NTPTime { _: Suite =>
  ...
  protected def withLevelDBWriter[A](bs: BlockchainSettings)(test: Blockchain with Storage => A): A = tempDb { db =>
  ...
  def withLevelDBWriter[A](fs: FunctionalitySettings)(test: Blockchain with Storage => A): A =
  ...
  def assertDiffEi(preconditions: Seq[Block], block: Block, state: Blockchain with Storage)(
      assertion: Either[ValidationError, Diff] => Unit
  ): Unit = {
