...
import com.wavesplatform.transaction.{GenesisTransaction, SignedTx}
...
class BlockRewardSpec extends FreeSpec with ScalaCheckPropertyChecks with WithDomain with Matchers with TransactionGen with NoShrink {
  ...
  "Miner receives reward and fees" - {
    val ngEmptyScenario = for {
      ...
      tx1 = SignedTx.transfer(1.toByte, issuer, sourceAddress.toAddress, Waves, 10 * Constants.UnitsInWave, Waves, OneTotalFee, None, ntpTime.getTimestamp())
      tx2 = SignedTx.transfer(1.toByte, issuer, sourceAddress.toAddress, Waves, 10 * Constants.UnitsInWave, Waves, OneTotalFee, None, ntpTime.getTimestamp())
    ...
    val betterBlockScenario = for {
      ...
      tx = SignedTx.transfer(1.toByte, issuer, sourceAddress.toAddress, Waves, 10 * Constants.UnitsInWave, Waves, OneTotalFee, None, ntpTime.getTimestamp())
