...
import com.wavesplatform.transaction.{Asset, GenesisTransaction, SignedTx}
...
class BlockchainUpdaterSponsoredFeeBlockTest
  ...
  val sponsorPreconditions: Gen[Setup] = for {
    ...
    masterToAlice = SignedTx.transfer(
        1.toByte,
        master,
        alice.toAddress,
        Waves,
    ...
    bobToMaster2 = SignedTx.transfer(
        1.toByte,
        bob,
        master.toAddress,
        Asset.fromCompatId(Some(feeAsset.id())),
