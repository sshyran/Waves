...
class BlockchainUpdaterMicroblockSunnyDayTest
  ...
  val preconditionsAndPayments: Gen[Setup] = for {
    ...
    aliceToBob  = createWavesTransfer(alice, bob.toAddress, masterToAlice.amount - fee - 1, fee, ts)
    aliceToBob2 = createWavesTransfer(alice, bob.toAddress, masterToAlice.amount - fee - 1, fee, ts + 1)
