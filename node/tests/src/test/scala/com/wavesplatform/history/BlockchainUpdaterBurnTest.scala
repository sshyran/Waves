...
import com.wavesplatform.transaction.{Asset, GenesisTransaction, SignedTx, TxVersion}
...
class BlockchainUpdaterBurnTest extends PropSpec with PropertyChecks with DomainScenarioDrivenPropertyCheck with Matchers with TransactionGen {
  ...
  val preconditions: Gen[Setup] = for {
    ...
    masterToAlice = SignedTx.transfer(1.toByte, master, alice.toAddress, Asset.Waves, 3 * Waves, Asset.Waves, transferAssetWavesFee, None, ts + 1)
