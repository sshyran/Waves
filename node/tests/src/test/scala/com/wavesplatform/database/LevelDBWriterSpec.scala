...
import com.wavesplatform.state.{Blockchain, BlockchainUpdaterImpl, Height, TransactionId, TxNum}
...
class LevelDBWriterSpec
  ...
  "hasScript" - {
    ...
    def test(f: (Blockchain with Storage, KeyPair) => Unit): Unit = baseTest(t => gen(t.correctedTime()))(f)

    def resetTest(f: (Blockchain with Storage, KeyPair) => Unit): Unit = baseTest(t => resetGen(t.correctedTime()))(f)

  }

  def baseTest(gen: Time => Gen[(KeyPair, Seq[Block])])(f: (Blockchain with Storage, KeyPair) => Unit): Unit = {
  ...
  def testWithBlocks(gen: Time => Gen[(KeyPair, Seq[Block])])(f: (Blockchain with Storage, Seq[Block], KeyPair) => Unit): Unit = {
    val defaultWriter = TestLevelDB.withFunctionalitySettings(db, ignoreSpendableBalanceChanged, TestFunctionalitySettings.Stub)
    val settings0     = WavesSettings.fromRootConfig(loadConfig(ConfigFactory.load()))
    val settings      = settings0.copy(featuresSettings = settings0.featuresSettings.copy(autoShutdownOnUnsupportedFeature = false))
    val bcu           = new BlockchainUpdaterImpl(defaultWriter, ignoreSpendableBalanceChanged, settings, ntpTime, ignoreBlockchainUpdateTriggers)
