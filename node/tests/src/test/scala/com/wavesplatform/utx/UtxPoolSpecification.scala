...
private object UtxPoolSpecification {
  ...
  final case class TempDB(fs: FunctionalitySettings, dbSettings: DBSettings) {
    val db: DB = new TestDB
    val writer = TestLevelDB.withFunctionalitySettings(db, ignoreSpendableBalanceChanged, fs)
...
class UtxPoolSpecification
  ...
  private def transfer(sender: KeyPair, maxAmount: Long, time: Time) =
    ...
    } yield SignedTx.transfer(1.toByte, sender, recipient.toAddress, Waves, amount, Waves, fee, None, time.getTimestamp()))
  ...
  private def transferV2(sender: KeyPair, maxAmount: Long, time: Time) =
    (for {
      ...
      recipient <- accountGen
      fee       <- chooseNum(extraFee, (maxAmount * 0.1).toLong)
    } yield SignedTx.transfer(TxVersion.V2, sender, recipient.toAddress, Waves, amount, Waves, fee, None, time.getTimestamp()))
  ...
  private val twoOutOfManyValidPayments = (for {
    ...
    val txs        = for (_ <- 1 to n) yield createWavesTransfer(sender, recipient.toAddress, amountPart, fee, time.getTimestamp())
  ...
  private def transactionV1Gen(sender: KeyPair, ts: Long, feeAmount: Long): Gen[TransferTransaction] = accountGen.map { recipient =>
    SignedTx.transfer(1.toByte, sender, recipient.toAddress, Waves, waves(1), Waves, feeAmount, None, ts)
  }

  private def transactionV2Gen(sender: KeyPair, ts: Long, feeAmount: Long): Gen[TransferTransaction] = accountGen.map { recipient =>
    SignedTx.transfer(2.toByte, sender, recipient.toAddress, Waves, waves(1), Waves, feeAmount, None, ts)
  }
    ...
    "event stream" - {
      "fires events correctly" in {
        val preconditions = for {
          ...
          validTransfer   = SignedTx.transfer(TxVersion.V1, richAcc, secondAcc.toAddress, Waves, 1, Waves, fee, None, ts)
          invalidTransfer = SignedTx.transfer(TxVersion.V1, secondAcc, richAcc.toAddress, Waves, 2, Waves, fee, None, ts)
