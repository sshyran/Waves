...
import com.wavesplatform.transaction.{DataTransaction, SignedTx, Transaction, TxVersion}
...
trait BlocksTransactionsHelpers { self: TransactionGen =>
  object QuickTX {
    ...
    def transfer(
      ...
      for {
        timestamp <- timestamp
      } yield SignedTx.transfer(1.toByte, from, to, Waves, amount, Waves, FeeAmount, None, timestamp)
    ...
    def transferV2(
      ...
      for {
        timestamp <- timestamp
      } yield SignedTx.transfer(2.toByte, from, to, Waves, amount, Waves, FeeAmount, None, timestamp)
    ...
    def transferAsset(
      ...
      for {
        timestamp <- timestamp
      } yield SignedTx.transfer(1.toByte, from, to, asset, amount, Waves, FeeAmount, None, timestamp)
