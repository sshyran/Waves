...
import com.wavesplatform.transaction.{CreateAliasTransaction, SignedTx, Transaction}
  ...
  test("wrong type of script return value") {
    ...
    assertBadRequestAndResponse(
      sender.signedBroadcast(
        SignedTx
          .transfer(2.toByte, acc0, acc1.toAddress, Waves, 1000, Waves, minFee + smartFee, None, ts)
          .json()
      ),
      "not a boolean"
    )
